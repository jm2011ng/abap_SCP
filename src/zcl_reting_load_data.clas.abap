CLASS zcl_reting_load_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_reting_load_data IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.

    DATA: lt_brands    TYPE TABLE OF zrent_brands_m,
          lt_cars      TYPE TABLE OF zrent_cars_m,
          lt_customers TYPE TABLE OF zrent_customer_m.
...
( marca = 'BMW' url = 'https://www.bmw.com.mx/content/dam/bmw/common/all-models/3-series/sedan/2018/navigation/bmw-3-series-modellfinder.png') ).
    ...
    DELETE FROM zrent_brands_m.
    INSERT zrent_brands_m FROM TABLE @lt_brands.

    SELECT * FROM zrent_brands_m INTO TABLE @lt_brands.
    out->write( sy-dbcnt ).
...
** CARS**
    ...
    DELETE FROM zrent_cars_m.
    INSERT zrent_cars_m FROM TABLE @lt_cars.
...
** CUSTOMERS**
    ...
    DELETE FROM zrent_customer_m.
    INSERT zrent_customer_m FROM TABLE @lt_customers.
    SELECT * FROM zrent_customer_m INTO TABLE @lt_customers.
    out->write( sy-dbcnt ).
